...
try:
    ...
    print('There is a problem during import of params.yaml for featurize subset')
    exit()
...
if "morganfingerprint" in mol_rep:
    ...
    train_df['fp']= train_df[X_id].progress_apply(lambda x: generate_fingerprint(x, radius, bits))
    train_df.to_csv(featurized_dir / "train.csv", index=False)
    
    print("\nFeaturizing Test Data")
    test_df['fp']= test_df[X_id].progress_apply(lambda x: generate_fingerprint(x, radius, bits))
    test_df.to_csv(featurized_dir / "test.csv", index=False)
